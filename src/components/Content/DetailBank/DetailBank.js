...
>>>>>>> ref button detail
  ...
  checkAccount = () => {
    this.props.onAccountDetail(
      this.props.dataDetail.name,
      this.state.bankNameVal,
      this.state.rekeningVal
    );
    this.setState({
    ...
    if (this.state.verified) {
      ...
      for (const key in this.props.dataDetail.accountTO) {
        if (
          this.props.dataDetail.accountTO[key].Bank_Code.toString() ===
          this.state.bankCode.toString()
        ) {
          success = true;
        }
      }
    ...
    for (const key in listAccount) {
      if (
        listAccount[key].No_Account.toString() ===
        this.state.rekeningVal
          .toString()
          .split("-")
          .join("")
  ...
  render() {
    ...
    let optionsForm = (
      <div className="popup-body">
        ...
        <div className="error-message jsNoResultsBankList">
    ...
    if (dataPropsBank) {
      ...
      if (listBank.length > 0) {
        ...
        optionsForm = (
          <option
            style={{ color: "red", fontSize: 12, textAlignLast: "center" }}
          >
            Tidak ada hasil ditemukan
          </option>
        );
      ...
      <div onClick={() => this.clickCard()}>
        ...
        <p className="amount">
          <strong className="color-primary">
            Rp {dataProps.amount.toLocaleString()}
          </strong>
        </p>
        <p>
          Silakan pilih bank dan rekening yang diinginkan untuk menyelesaikan
          proses transfer
        </p>
        <div className="detail-bank-form">
          <form
            action="transfer-success.html"
            className="form jsFormDetailBank"
            autoComplete="off"
          >
            <div className="row">
              <div className="col-lg-6 col-sm-12">
                <div
                  className={this.state.inputClassFormBank}
                  onClick={event => this.clickformOpt(event)}
                >
                  <input
                    name="bank_name"
                    type="text"
                  ...
                  <input
                    name="bank_code"
                    type="hidden"
                    className="jsBankCode"
                  />
                  <label htmlFor="keyInBank" className="input-label">
                    Masukkan Nama Bank
                  </label>
                </div>

                <div
                  className={this.state.inputClassListBank}
                  data-jsonurl="data/bank-list.json"
                >
                  {/* HEADER */}
                  <div className="popup-header">
                    ...
                    <div className="popup-close jsCloseBankDataPopup">
                      Close Icon
                    </div>
                  </div>
...
>>>>>>> ref button detail
            ...
            <div className={this.state.showName}>
              {this.state.targerTransfer} &nbsp;
            </div>
            <div className="bank-reset-form" onClick={() => this.resetForm()}>
              <div
                className="not-this-account jsResetDetailBank"
...
const mapDispatchToProps = dispatch => {
  return {
    onAccountDetail: (targerTransfer, bankNameVal, rekeningVal) =>
      dispatch(
        actions.getOnAccountDetail(targerTransfer, bankNameVal, rekeningVal)
      )
  };
};
